...
def compute_weighted_representation(lines: List[LineInfo]):
    word_statistics: Dict[str, List[LineInfo.WordStatistics]] = {}
    line_to_term_frequencies: Dict[int, Dict[str, LineInfo.WordStatistics]] = {}
    for line_info in lines:
        line_word_statistics = {}
        line_info.fill_word_statistics(line_word_statistics)
        line_to_term_frequencies[line_info.number] = line_word_statistics
        for word, word_statistic in line_word_statistics.items():
            if word not in word_statistics:
                word_statistics[word] = []
                word_statistics[word].append(word_statistic)

    # compute IDF(w_j)
    word_to_inverse_document_frequency = compute_inverse_document_frequency(word_statistics, document_count=len(lines))
    words = word_statistics.keys()
    ...
    for line_info in lines:
        ...
        term_frequencies_for_line: Dict[str, LineInfo.WordStatistics] = line_to_term_frequencies[line_info.number]
...
def print_line_vector(line):
    text = ''
    zero_sequence = 0
    for element in line.vector:
        if element == 0:
            zero_sequence += 1
            continue
        if text:
...
def main():
    ...
    compute_weighted_representation(train_lines)
    print_line_vector(train_lines[0])
